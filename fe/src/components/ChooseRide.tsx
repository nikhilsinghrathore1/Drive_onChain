...
interface val {
  fare:{}
  setvehical:(val:string)=>void,
  func1 : (value:boolean) =>void,
  finalfare :(val:string) =>void,
  func2 : (value:boolean) =>void
}


const ChooseRide:React.FC<val> = ({setvehical,fare,func1 , func2 , finalfare}) => {
  return (
               ...
               <div onClick={()=>(func2(true),func1(false) ,finalfare(fare?.car) ,setvehical("car"))} className="flex w-full h-[11vh] my-3 items-center active:border-4 border-[1px] border-black rounded-xl justify-between px-5 py-2">
             ...
             <div >
               <h1 className="text-2xl font-bold">${fare?.car}</h1>
         
             </div>
         
               </div>
               <div onClick={()=>(func2(true),func1(false) ,finalfare(fare?.auto),setvehical("auto"))} className="flex w-full h-[11vh] my-3 items-center active:border-4 border-[1px] border-black rounded-xl justify-between px-5 ">
                 {/* this is the image div  */}
             ...
             <div >
               <h1 className="text-2xl font-bold">${fare.auto}</h1>
         
             </div>
         
               </div>
               <div onClick={()=>(func2(true),func1(false) ,finalfare(fare?.bike) , setvehical("bike"))} className="flex w-full h-[11vh] my-3 items-center active:border-4 border-[1px] border-black rounded-xl justify-between px-5 ">
                 {/* this is the image div  */}
                 <div className="w-[25%] h-full flex items-center overflow-hidden " >
             ...
             <div >
               <h1 className="text-2xl font-bold">${fare.bike}</h1>
